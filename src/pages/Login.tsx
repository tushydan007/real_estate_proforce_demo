...
function getErrorMessage(err: unknown): string {
  if ((err as AxiosError)?.isAxiosError) {
    ...
    // Check for specific error messages in order of preference
    if (respData?.detail) return respData.detail;
    if (respData?.non_field_errors?.[0]) return respData.non_field_errors[0];
    if (respData?.email?.[0]) return `Email: ${respData.email[0]}`;
    if (respData?.password?.[0]) return `Password: ${respData.password[0]}`;
    ...
    switch (status) {
      ...
      default:
        return axiosErr.message || "Login failed";
    }
  }
...
export default function Login() {
  ...
  async function onSubmit(values: LoginFormData) {
    try {
      ...
      // Use Redux action to store token
          ...
          >
            <div className="flex flex-col items-center gap-2 pt-4 sm:flex-row sm:justify-between sm:gap-6">
              ...
              >
                ...
                <span className="font-medium text-gray-300 hover:text-blue-400">
                  Register
                </span>
              </Link>
            </div>
...
//               >
//                 Don't have an account?{" "}
//                 <span className="font-medium text-gray-300 hover:text-blue-400">Register</span>
//               </Link>
//             </div>
//           </motion.div>
//         </motion.form>
