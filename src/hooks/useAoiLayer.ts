...
const isCircle = (geometry: GeoJSON.Polygon | GeoJSON.MultiPolygon): boolean =>
...
const circleToPolygon = (
  ...
  type: "Polygon",
  coordinates: [
    getCirclePoints(circle.getLatLng(), circle.getRadius(), segments),
  ],
});
        ...
        for (const [i, feature] of validFeatures.entries()) {
          ...
          if (this.options.autoAddEnabled) {
            ...
            const newAoiCartItem: AoiCartItem = {
              ...
              type: this.options.isRectangle(geom)
                ? "Rectangle"
                : this.options.isCircle(geom)
                ? "Circle"
                : geom.type,
              is_active: true,
  ...
  useEffect(() => {
    ...
    const handleCreated = async (event: L.LeafletEvent) => {
      ...
      if (e.layerType === "circle") {
        ...
        geoJson = layer.toGeoJSON().geometry as
          | GeoJSON.Polygon
