...
// /**
//  * Calculate the centroid of a geometry for geocoding
//  */
...
//   return pointCount > 0 ? [xSum / pointCount, ySum / pointCount] : [0, 0];
// };
...
// ): Promise<string> => {
//   try {
//     const [lng, lat] = getGeometryCentroid(geometry);
...
//               (await this.options.generateAoiName(geom, startIndex + i));
...
//         const aoiName = await generateAoiName(geoJson, aois.length + 1);
//         const newAoiCartItem: AoiCartItem = {
//           id: tmpId,
//           name: aoiName,
//           geometry: geoJson,
...
//               id: aoi.id!,
//               name: aoi.name || (await generateAoiName(aoi.geometry, aoi.id!)),
...
//               (async () => {
//                 const resolvedName =
//                   aoi.name || (await generateAoiName(aoi.geometry, aoiId));
//                 const cartItem: AoiCartItem = {
//                   id: aoiId,
//                   name: resolvedName,
//                   geometry: aoi.geometry,
...
const isCircle = (
  ...
  geometry.type === "Polygon" && geometry.coordinates[0].length >= 65;

/**
 * Convert a Leaflet Circle to a GeoJSON Polygon approximation
 */
  ...
  for (let i = 0; i < segments; i++) {
    ...
    const lat2 = Math.asin(
      Math.sin(lat1) * Math.cos(radius / earthRadius) +
        Math.cos(lat1) * Math.sin(radius / earthRadius) * Math.cos(brng)
    );
    const lon2 =
      lon1 +
      Math.atan2(
        Math.sin(brng) * Math.sin(radius / earthRadius) * Math.cos(lat1),
...
const circleToPolygon = (
  ...
  type: "Polygon",
  coordinates: [getCirclePoints(circle.getLatLng(), circle.getRadius(), segments)],
});
...
interface UploadControlOptions extends L.ControlOptions {
  ...
  isRectangle: (geometry: GeoJSON.Polygon | GeoJSON.MultiPolygon) => boolean;
  isCircle: (geometry: GeoJSON.Polygon | GeoJSON.MultiPolygon) => boolean;
  calculateAreaFromGeometry: (
    geometry: GeoJSON.Polygon | GeoJSON.MultiPolygon
  ) => number;
  getAoisCount?: () => number;
        ...
        for (const [i, feature] of validFeatures.entries()) {
          ...
          if (this.options.autoAddEnabled) {
            ...
            const newAoiCartItem: AoiCartItem = {
              ...
              area,
              coordinates: this.options.formatCoordinates(geom),
              type: this.options.isRectangle(geom) ? "Rectangle" : this.options.isCircle(geom) ? "Circle" : geom.type,
              is_active: true,
              monitoring_enabled: false,
              created_at: new Date().toISOString(),
              addedToCartAt: new Date().toISOString(),
...
export const useAoiLayer = ({
  ...
  useEffect(() => {
    ...
    const handleCreated = async (event: L.LeafletEvent) => {
      ...
      if (!["polygon", "rectangle", "circle"].includes(e.layerType)) return;

      let geoJson: GeoJSON.Polygon | GeoJSON.MultiPolygon;
      if (e.layerType === "circle") {
        if (!(layer instanceof L.Circle)) return;
        geoJson = circleToPolygon(layer);
      } else {
        if (!(layer instanceof L.Polygon)) return;
        geoJson = layer.toGeoJSON().geometry as GeoJSON.Polygon | GeoJSON.MultiPolygon;
      }
      ...
      if (autoAddEnabled) {
        ...
        const newAoiCartItem: AoiCartItem = {
          ...
          area,
          coordinates: formatCoordinates(geoJson),
          type: isRectangle(geoJson) ? "Rectangle" : isCircle(geoJson) ? "Circle" : geoJson.type,
          is_active: true,
          monitoring_enabled: false,
          created_at: new Date().toISOString(),
          addedToCartAt: new Date().toISOString(),
        ...
        layer.on("click", async (evt) => {
          ...
          if (isInCart) {
            ...
            const cartItem: AoiCartItem = {
              ...
              area,
              coordinates: formatCoordinates(aoi.geometry),
              type: isRectangle(aoi.geometry) ? "Rectangle" : isCircle(aoi.geometry) ? "Circle" : aoi.geometry.type,
              is_active: aoi.is_active,
              monitoring_enabled: aoi.monitoring_enabled,
              created_at:
                typeof aoi.created_at === "string"
            ...
            if (present) {
              ...
              (async () => {
                ...
                const cartItem: AoiCartItem = {
                  ...
                  type: isRectangle(aoi.geometry)
                    ? "Rectangle"
                    : isCircle(aoi.geometry) ? "Circle" : aoi.geometry.type,
                  is_active: aoi.is_active,
                  monitoring_enabled: aoi.monitoring_enabled,
                  created_at:
                    typeof aoi.created_at === "string"
  ...
  const startCircle = useCallback(() => {
    if (!map) return;
    const drawer = new L.Draw.Circle(map as DrawMap, {
      shapeOptions: { color: "blue", weight: 2, fillOpacity: 0.2 },
    });
    drawer.enable();
  }, [map]);
